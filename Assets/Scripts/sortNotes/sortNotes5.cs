...
public class sortNotes5 : MonoBehaviour
{

    public List<GameObject> noteList5 = new List<GameObject>();
    ...
    void rowSelector()
    {
        if (selectedRow == SelectedRow.row5)
        {
            foreach (GameObject singleNote in GameObject.FindGameObjectsWithTag("Note"))
            {

                if (singleNote.GetComponent<Transform>().position.x == r5)
                {
                    noteList5.Add(singleNote);
                }
        ...
        foreach (GameObject singleNote in noteList5) //THIS IS CHECKING THE LOWEST Z POSITION
        {
        ...
        foreach (GameObject singleNote in noteList5) // THIS ONE IS ASSIGNING THE LOWEST SCORE TO ONE NOTE
        {
            if (singleNote.GetComponent<Transform>().position.z == min)
            {
