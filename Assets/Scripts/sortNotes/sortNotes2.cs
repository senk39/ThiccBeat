...
public class sortNotes2 : MonoBehaviour
{
    ...
    void rowSelector()
    {
        if (selectedRow == SelectedRow.row2)
        {
        ...
        foreach (GameObject singleNote in noteList2) //THIS IS CHECKING THE LOWEST Z POSITION
        {
        ...
        foreach (GameObject singleNote in noteList2) // THIS ONE IS ASSIGNING THE LOWEST SCORE TO ONE NOTE
        {
            if (singleNote.GetComponent<Transform>().position.z == min)
            {
