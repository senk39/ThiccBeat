...
public class sortNotes6 : MonoBehaviour {

    public List<GameObject> noteList6 = new List<GameObject>();
    ...
    void rowSelector()
    {
        if (selectedRow == SelectedRow.row6)
        {
            foreach (GameObject singleNote in GameObject.FindGameObjectsWithTag("Note"))
            {
               
                if (singleNote.GetComponent<Transform>().position.x == r6)
                {
                    noteList6.Add(singleNote);
                }
        ...
        foreach (GameObject singleNote in noteList6) //THIS IS CHECKING THE LOWEST Z POSITION
        {
        ...
        foreach (GameObject singleNote in noteList6) // THIS ONE IS ASSIGNING THE LOWEST SCORE TO ONE NOTE
        {
            if (singleNote.GetComponent<Transform>().position.z == min)
            {
